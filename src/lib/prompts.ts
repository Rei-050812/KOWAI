...
- 登場人物の心理を深く描く
- 10〜15段落に分けて改行を入れる
- 水増し禁止：長さのための冗長な描写は入れない。全ての描写に意味を持たせる`,
};
...
【文体統一ルール（最重要）】
- この1文の文体が、続くPhase B/Cと統一されていなければならない
- 下記に「書き方指示」がある場合、その指示に従った文体で書くこと
- 「書き方指示」がない場合は「だ・である」調の過去形を使用
- Phase A の文体が浮くと全体の統一感が損なわれるため、必ず守ること
...
function getPhaseStyleGuide(style: StoryStyle, phase: 'A' | 'B' | 'C'): string {
  const lengthGuides: Record<StoryStyle, Record<'A' | 'B' | 'C', string>> = {
    short: {
      ...
      B: '約400字。違和感を端的に。水増し禁止。',
      C: '約350字。不可逆点+短いクライマックス+余韻。',
    },
    medium: {
      A: '1文のみ（最大50字）。時間・場所・立場の事実だけ。',
      B: '約750字。違和感を丁寧に積み上げ。冗長な繰り返し禁止。',
      C: '約700字。不可逆点+クライマックス（パニック/顕在化/脱出）+余韻。',
    },
    long: {
      A: '1文のみ（最大50字）。時間・場所・立場の事実だけ。',
      B: '約1500字。違和感を複数の描写で積み上げ。全ての描写に意味を持たせる。',
      C: '約1400字。不可逆点+クライマックス（パニック/顕在化/脱出）+余韻。丁寧だが冗長にならない。',
    },
  };
...
export function buildStyleHint(style: StyleBlueprintData | null): string {
  ...
  return `
【書き方指示（流派: ${style.archetype_name}）※Phase A/B/C 全てで統一すること】
視点: ${stanceLabel}
感情: ${emotionLabel}
文体: ${sentenceLabel}
会話: ${dialogueLabel}
...
※ 冒頭の1文（Phase A）から終盤（Phase C）まで、上記の文体で一貫させること。途中で文体が変わると読者に違和感を与える。
`;
}
