...
- ✨ **品質改善機能**
  ...
  - **語彙クールダウン**: 直近5話で頻出した語彙（曜日・家族・時間帯・道具・場所・感情表現等50種以上）を非推奨リストとして渡し、多様性を確保
  - **キーワード連呼防止**: Phase A=1〜2回、Phase B=最大2回、Phase C=最大1回の制限で過剰なキーワード使用を防止
  - **クライマックス検証**: 恐怖ピーク語彙・余韻文の存在をチェック
  - **キーワード主役化**: キーワードが怪異の核として機能しているか検証
  - **未完セリフ検出**: 言いかけのセリフ（「…」「―」で終わる）を検出
  - **怪異セリフ品質ルール**: 陳腐なホラーセリフ（「やっと出られる」「一緒に遊ぼう」等）を禁止し、文脈から浮いた不気味なセリフを生成
...
- 📝 **3つのスタイル**
  - 短編（約800字）：サクッと読める、オチ重視
  - 中編（約1500字）：雰囲気が徐々に積み重なる
  - 長編（約3000字）：本格怪談、伏線や複数の展開

- 🎲 **おまかせモード**
  - 単語を入力せずに怪談を生成可能
  - ランダムなBlueprintを自動選択して生成
...
- 🔒 **コンテンツ品質管理**
  ...
  - **連続低評価StyleBlueprintの自動無効化**（連続3回 rating ≤ 2 で is_active = false）
...
│       └── 016_auto_deactivate_low_rated_style.sql # 連続低評価StyleBlueprint自動無効化
├── public/                    # 静的ファイル
...
タブ切替で構造（KaidanBlueprint）と文体（StyleBlueprint）を一括管理。ログイン時に自動読み込み。

- **構造タブ**: KaidanBlueprint一覧表示・単体新規登録・削除
- **文体タブ**: StyleBlueprint一覧表示・編集・有効/無効切替・単体新規登録・削除
  - 流派名の類似チェック機能（既存と似た名前を自動回避）
...
- 抽出結果をプレビューし、**その場で直接登録可能**（構造・文体それぞれ個別に）
...
- **全フェーズ（A/B/C）の生成プロンプトに注入**され、冒頭から文体の一貫性を確保
- 品質スコア・使用回数・平均評価による自動選別
- レビュー評価が `avg_story_rating` に自動集計され、高評価スタイルが優先選択される

#### レビュー → 生成 フィードバックループ

```
ストーリー生成 → レビュー評価(1-5) + 問題タグ → StyleBlueprintのスコア更新
       ↑                                              ↓
       ← ← ← ← 高スコアスタイルが優先選択 ← ← ← ← ←
```
...
**選択スコア計算式：**
```
スコア = (未使用期間ボーナス) + (平均評価×10) + (品質スコア×0.3)
       - (支離滅裂率×25) - (行動不整合率×20) - (出来事再描写率×15)
       - (未完セリフ率×10) - (レビュー問題率×15)
```

- 高評価・低問題スタイル → 選択スコア上昇 → より多く使用される
- 低評価・高問題スタイル → 選択スコア低下 → 自動的に淘汰される
- **連続3回低評価（rating ≤ 2）で自動無効化**（is_active = false）

#### 低評価ストーリーの文章学習

選択されたStyleBlueprintで過去に低評価(1-2)を受けたストーリーがある場合、その文章の抜粋（冒頭・中盤・終盤から各100文字）を「避けるべき例」としてプロンプトに注入します。

```
【避けるべき表現例】以下は過去に低評価だった文章の抜粋（冒頭・中盤・終盤）です。
同様の表現パターンは避けてください：
悪例1: 「火曜日の夜だった。... 何度も火曜日と繰り返し... また火曜日に戻ってきた。」
悪例2: 「お母さんに怒られる。... お母さんの顔が... お母さんは笑っていた。」
```

これにより、同じスタイルでも過去の失敗パターンを繰り返さなくなります。
...
| 語彙クールダウン | 全フェーズ | 直近5話で頻出した語彙（50種以上）を非推奨リストとしてプロンプトに渡す |
| キーワード連呼防止 | 全フェーズ | Phase A=1〜2回、Phase B=最大2回、Phase C=最大1回に制限 |
