...
- 🔄 **3フェーズ生成システム**
  - **Phase A（opening）**: 日常の前提と状況を淡々と描写
  - **Phase B（disturbance）**: 違和感・怪異の兆候を1つだけ描写
...
|---------|------|---------------------|---------|
| **Phase A** | 日常の前提を淡々と描写 | `normal_rule`, `detail_bank` | 怪異の示唆、不穏な表現 |
| **Phase B** | 違和感・怪異の兆候を1つ描写 | `anomaly` | 説明、原因、怪異の増殖 |
...
各フェーズで以下のバリデーションを実行し、失敗時は自動リトライします：
...
| 導入文被りチェック | Phase B | Phase Aと同じ冒頭文型（「俺が〜」等）で始まっていないか（リトライ付き） |
